            ...
            ForEach((-viewConductor.layoutRowsCols.rowsPerSide[.symmetric]!...viewConductor.layoutRowsCols.rowsPerSide[.symmetric]!).reversed(), id: \.self) { row in
                HStack(spacing: 0) {
                    ForEach(viewConductor.lowMIDI...viewConductor.highMIDI, id: \.self) { col in
                        ...
                        let majorMinor: MajorMinor = Interval.majorMinor(midi: unSureMIDI, tonicMIDI: Int(TonalContext.shared.tonicMIDI))
                        if majorMinor == .minor {
                            VStack(spacing: 0.0)  {
                                if TonalContext.shared.safeMIDI(midi: unSureMIDI + 1) {
                                    KeyboardKeyContainerView(conductor: viewConductor,
                                                             pitch: TonalContext.shared.allPitches[unSureMIDI + 1],
                                ...
                                if TonalContext.shared.safeMIDI(midi: unSureMIDI) {
                                    KeyboardKeyContainerView(conductor: viewConductor,
                                                             pitch: TonalContext.shared.allPitches[unSureMIDI],
                                                             keyboardKeyView: keyboardKeyView)
                                } else {
                                    Color.clear
                                }
                            ...
                            let intervalClass: IntegerNotation = Interval.intervalClass(midi: unSureMIDI, tonicMIDI: Int(TonalContext.shared.tonicMIDI))
                            if intervalClass == .seven { // perfect fifth takes care of rendering the tritone above it
                                if TonalContext.shared.safeMIDI(midi: unSureMIDI) {
                                    ...
                                    .overlay() { // render tritone as overlay
                                        ...
                                        if TonalContext.shared.safeMIDI(midi: unSureMIDI - 1) && TonalContext.shared.safeMIDI(midi: unSureMIDI - 2) {
                                            GeometryReader { proxy in
                                                let ttLength = viewConductor.tritoneLength(proxySize: proxy.size)
                                                ZStack {
                                                    KeyboardKeyContainerView(conductor: viewConductor,
                                                                             pitch: TonalContext.shared.allPitches[unSureMIDI-1], // tritone
                                                                             zIndex: 1,
                                                                             keyboardKeyView: keyboardKeyView)
